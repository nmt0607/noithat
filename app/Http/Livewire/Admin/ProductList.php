...
class ProductList extends BaseLive
{
    ...
    public function render(){
        ...
        $this->pdTypeParentListLv1 = ProductType::where('parent_id','!=', 0)->get()->pluck('name', 'id');
        $query = Product::query();
        ...
        if($this->searchType) {
            $query->where("pd_type_lv2",$this->searchType);
        }

        if($this->searchTypeLv1) {
            $query->where("pd_type_lv1",$this->searchTypeLv1);
        }

        $data = $query->orderBy($this->key_name,$this->sortingName)->paginate($this->perPage);
        return view('livewire.admin.product-list', [
            'data'=> $data,
            'pdTypeParentList'=> $this->pdTypeParentList,
            'pdTypeParentListLv1'=> $this->pdTypeParentListLv1,
