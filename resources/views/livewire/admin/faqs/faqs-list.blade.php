    ...
    <div class="p-2 pb-3 d-flex align-items-center justify-content-between">
        ...
        <div class="paginate">
            <div class="d-flex">
                ...
                <div class="">
                    <div class="disable">Câu hỏi thường gặp</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group"  style='margin:0 0 5px;'>
                            ...
                            <input type="text" class="form-control"
                                placeholder="Câu hỏi" wire:model.defer="question">
                            @error('question')
                                @include('layouts.partials.text._error')
                            @enderror
                        </div>
                        <div class="form-group"  style='margin:0 0 5px;' wire:ignore>
                            <label> Câu trả lời(<span style="color:red">*</span>)</label>
                            <textarea  wire:model.defer="answer" id="content" rows="5" cols="100"></textarea>
                            @error('answer')
                                @include('layouts.partials.text._error')
                            @enderror
                        </div>
                        <div class="form-group"  style='margin:0 0 5px;'>
                            ...
                            <input type="text" class="form-control"
                                placeholder="Sản phẩm áp dụng" wire:model.defer="category">
                            @error('category')
                                @include('layouts.partials.text._error')
                            @enderror
                        </div>
                        <div class="form-group"  style='margin:0 0 5px;'>
                            ...
                            <input type="number" class="form-control col-6" oninput="this.value = Math.abs(this.value)"
                                placeholder="Số thứ tự" wire:model.defer="order_number">
                            @error('order_number')
                                @include('layouts.partials.text._error')
                            @enderror
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" @if($mode == 'detail') disabled @endif class="btn btn-primary" id='btn_save'>
                            Lưu
                        </button>
                    </div>
                </div>
...
<script>
    $("document").ready(() => {
        ...
        window.livewire.on('setEditor', (data) => {
            $('#content').summernote('code', data);
        });
        $('#btn_save').click(function() {
            window.livewire.emit('setAnswer', $('#content').summernote('code'));
        })
    });
</script>
